...
bool Monster::isHaveCoollideFromHero() {
	...
	for (int i = tileSize.width / 2; i * i < disSqu; i += (tileSize.width / 2)) {
...
bool Monster::setPath() {
	...
	for (auto wayPoint : listPath) {
		...
		Sprite* sprite = Sprite::create("npc.png");
		//sprite->setScale(0.2f);
		sprite->setPosition(tilePosToCenterPos(wayPoint, map));

		getParent()->addChild(sprite , 3);
	}
